...
import { getCoreJobById, Job } from '@/lib/data/jobs'; // Use Core function and Job type
...
function ApplyPageContent() {
    ...
    useEffect(() => {
        if (jobId) {
            const coreJob = getCoreJobById(jobId);
            if (coreJob) {
                // Hydrate the job object just like in the detail page
                const hydratedJob: Job = {
                    ...coreJob,
                    title: getMessage('jobs', `${coreJob.jobId}_title`) || '',
                    // The form only needs the title, but you could hydrate more if needed
                    tags: [], summary: { location: '', salary: '', type: '' },
                    description: '', responsibilities: [], qualifications: [], benefits: []
                };
                setJob(hydratedJob);
            }
        }
    }, [jobId, locale, getMessage]);

    if (!job) {
        return <div className={styles.loading}>Loading...</div>;
    }
    
    // The uiTextForForm doesn't need changes, but we add fallbacks just in case
    const uiTextForForm = {
      ...
      // ... add || '' to all other uiText fields
